...
void car(){
    ...
    glTranslatef(position11,0.0f, 0.0f);
        ...
        glVertex2f(-0.855f, -0.66f);
        glVertex2f(-0.885f, -0.73f);
        glVertex2f(-0.815f, -0.79f);
        ...
        glEnd();
         ...
         glBegin(GL_QUADS);//tank upper join
        ...
        glBegin(GL_QUADS);//tank gun stand
        glColor3f(0.0f, 0.0f, 0.0f);
...
void displayProposal()
{
    ...
    renderBitmapString(-0.18f, 0.80f, 0.0f, GLUT_BITMAP_TIMES_ROMAN_24, "Computer Graphics Project [I]");
    renderBitmapString(-0.31f, 0.70f, 0.0f, GLUT_BITMAP_TIMES_ROMAN_24, "Project Title: Liberation's Canvas: A Graphic Tale of Freedom's War ");
    renderBitmapString(-0.20f, 0.60f, 0.0f, GLUT_BITMAP_TIMES_ROMAN_24, "Course teacher: Dipta Justin Gomes");
    renderBitmapString(-0.5f, 0.50f, 0.0f, GLUT_BITMAP_TIMES_ROMAN_24, "Group Members:");

    renderBitmapString(-0.5f, 0.40f, 0.0f, GLUT_BITMAP_TIMES_ROMAN_24, "1.Name: Shakil Jamil Mrida");
    renderBitmapString(-0.5f, 0.30f, 0.0f, GLUT_BITMAP_TIMES_ROMAN_24, "ID: 22-47275-1");

    renderBitmapString(-0.5f, 0.20f, 0.0f, GLUT_BITMAP_TIMES_ROMAN_24, "2.Name: Nowrin Tasnim");
    renderBitmapString(-0.5f, 0.10f, 0.0f, GLUT_BITMAP_TIMES_ROMAN_24, "ID: 22-47280-1");

    renderBitmapString(-0.5f, 0.00f, 0.0f, GLUT_BITMAP_TIMES_ROMAN_24, "3.Name: Hasibur Rahman Rahim");
    renderBitmapString(-0.5f, -0.10f, 0.0f, GLUT_BITMAP_TIMES_ROMAN_24, "ID: 22-47044-1");
...
glBegin(GL_POLYGON);
	...
	glBegin(GL_POLYGON);  // These vertices form a closed polygon
    glColor3f(0.0f, 0.9f, 0.2f); //Green
	glVertex2f(-1.0f, -0.20f);
    ...
    ///human2
    glBegin(GL_QUADS);  // body
    glColor3f(0.0f, 1.0f, 0.0f);//peach
	...
	glBegin(GL_QUADS);  // gun
    glColor3f(0.0f, 0.0f, 0.0f);//purple
    ...
    glColor3f(0.0f, 0.0f, 1.0f);//purple
    ...
    glTranslatef(position4,0.0f, 0.0f);
        GLfloat x35=-0.14; GLfloat y35=-0.43f; GLfloat radius35 =-0.005f;
        glColor3f(0.0f, 0.0f, 0.0f);
        glBegin(GL_TRIANGLE_FAN);
	...
	glBegin(GL_TRIANGLE_FAN);
      ...
      //blood
      GLfloat x100=0.38f; GLfloat y100=-0.39f; GLfloat radius100=0.009f;
	...
	glBegin(GL_TRIANGLE_FAN);
		...
		for(i = 0; i <= triangleAmount;i++) {
			...
			glVertex2f( x100 + (radius100 * cos(i *  twicePi / triangleAmount)),

                        y100+ (radius100 * sin(i * twicePi / triangleAmount)) );
	...
	GLfloat x67=0.16f; GLfloat y67=-0.07f; GLfloat radius67=0.02f;
    ...
    glVertex2f(0.12f, -0.1f);    // End point of the line
    glEnd();
...
///river tree///
   ...
   //human blood
	...
	//blood
	GLfloat x64=-0.10f; GLfloat y64=-0.35f; GLfloat radius64=0.02f;
...
void handleKeypress(unsigned char key, int x, int y) {
	switch (key) {
	    ...
	    case  'd':  ///day
            ...
            glutCreateWindow("Liberation's Canvas: A Graphic Tale of Freedom's War (Day View)");
         ...
         case 'n':  ///night
	        ...
	        glutInitWindowSize(1920, 1080);
            glutInitWindowPosition((glutGet(GLUT_SCREEN_WIDTH)-1920)/2,(glutGet(GLUT_SCREEN_HEIGHT)-1080)/2);
            glutCreateWindow("Liberation's Canvas: A Graphic Tale of Freedom's War (Night View)");
            glutKeyboardFunc(handleKeypress);
...
int main(int argc, char** argv)
{
	...
	glutCreateWindow("Computer Graphics [i]");  // Create window with the given title
	...
	glutTimerFunc(100, update4, 0);     //moving bullet
	...
	glutTimerFunc(100, update11, 0);    //moving tank
